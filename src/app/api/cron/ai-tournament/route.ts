...
// v2.0 - Implementing AI Personalities
...
const OPENING_BOOK = {
  "e4": { "e5": { "Nf3": { "Nc6": {} } } }, // Apertura Italiana
  "d4": { "d5": { "c4": { "e6": {} } } },   // Gambito de Dama
};
...
function getBestMove(game: Chess, personality: string, opponentPersonality: string, moveNumber: number) {
  // Lógica para el Erudito (KernelKing)
  if (personality === 'OPENING_BOOK' && moveNumber <= 4) {
    const history = game.history();
    let bookMoves = OPENING_BOOK as any;
    for (const move of history) {
      if (bookMoves[move]) bookMoves = bookMoves[move];
      else { bookMoves = null; break; }
    }
    if (bookMoves && Object.keys(bookMoves).length > 0) {
      const bookMove = Object.keys(bookMoves)[0];
      const moveObject = game.moves({ verbose: true }).find(m => m.san === bookMove);
      if (moveObject) return moveObject;
    }
  }

  // Lógica para el Caótico (QuantumLeap)
  ...
  // Lógica para el Adaptable (CodeCaster)
  let currentPersonality = personality;
  if (personality === 'ADAPTIVE') {
    if (opponentPersonality.includes('AGGRESSIVE') || opponentPersonality.includes('BERSERKER')) {
      currentPersonality = 'DEFENSIVE';
    } else if (opponentPersonality.includes('DEFENSIVE') || opponentPersonality.includes('FORTRESS')) {
      currentPersonality = 'AGGRESSIVE';
    }
  }
  ...
  for (const move of legalMoves) {
    ...
    switch (currentPersonality) {
      case 'BERSERKER':
        if (move.flags.includes('c')) score *= 2.0;
        break;
      case 'AGGRESSIVE':
        if (move.flags.includes('c')) score *= 1.5;
        break;
      case 'PRESSURER':
        // Lógica simple para control central
        if (['e4', 'd4', 'e5', 'd5'].includes(move.to)) score += 5;
        break;
      case 'DEFENSIVE':
...
function simulateGame(p1: any, p2: any, startTime: Date): { winner: 'p1' | 'p2', moves: { move: string, timestamp: string }[] } {
...
export async function GET(request: Request) {
  ...
  try {
    // 1. Crear/Actualizar las IAs con sus personalidades
    const newAIs = Object.entries(AI_PERSONALITIES).map(([name, personality]) => ({
      email: `${name.toLowerCase()}@system.io`,
      name,
      isAI: true,
      personality,
    }));
    await supabaseAdmin.from('ChessPlayer').upsert(newAIs, { onConflict: 'email' });
    ...
    // 4. Si hay torneo, comprobar si la ronda ha terminado
    ...
    // 5. Si no hay partidas activas, avanzar
