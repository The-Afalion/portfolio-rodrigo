...
const aiConfigByDifficulty: Record<
  Dificultad,
  { elo: number; estilo: "agresivo" | "defensivo" | "equilibrado" | "caotico" }
> = {
  Fácil: { elo: 800, estilo: "defensivo" },
  Medio: { elo: 1200, estilo: "equilibrado" },
  Difícil: { elo: 1800, estilo: "agresivo" },
};
...
export default function ChessGame() {
  ...
  const realizarMovimientoIA = useCallback((partidaActual: Chess) => {
    const config = aiConfigByDifficulty[dificultad];
    const movimientoIA = obtenerMovimientoIA(partidaActual, config.elo, config.estilo);
    if (movimientoIA) {
